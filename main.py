...
def user_page(user_id, page_num):
    ...
    with engine.connect() as conn:
        pb_game_query = conn.execute('select id, player_id, replay_player_id from game where round(game_time, 4) = ' + str(user_data["time"]) + ' and player_id = ' + str(user_id)).fetchall()
